...
namespace LINQSamples
{
    public class SamplesViewModel : ViewModelBase
    {
        ...
        public List<SalesOrder> ForEachQuery()
        {
            ...
            (from sale in sales let temp = sale.LineTotal = sale.OrderQty * sale.UnitPrice select sale).ToList();
        ...
        public List<Product> ForEachSubQueryQuery()
        {
            ...
            // Write Query Syntax Here
            (from prod in products let temp = prod.TotalSales = sales.Where(sale=>sale.ProductID==prod.ProductID).Sum(sale=>sale.LineTotal) select prod).ToList();
        ...
        public List<Product> ForEachSubQueryMethod()
        {
            ...
            // Write Method Syntax Here
            products.ForEach(p => p.TotalSales = sales.Where(sale => sale.ProductID == p.ProductID).Sum(sale => sale.LineTotal));
        ...
        public List<Product> ForEachQueryCallingMethodQuery()
        {
            ...
            // Write Query Syntax Here
            var list = (from prod in products let temp = prod.TotalSales = CalculateTotalSalesForProduct(prod,sales) select prod);
            list.Where(p => p.TotalSales > 0);
